    ...
    <AdminShell title="联系表单管理" description="前台 Contact 表单已真实写入 contact_messages；此处可搜索、按状态/分类/语言筛选、更新处理状态与备注，并导出当前筛选结果 CSV。">
        ...
        <form className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          <label className="min-w-0 md:col-span-2 xl:col-span-5">
            ...
            <div className="flex min-h-12 items-center gap-3 border border-[color:var(--border)] bg-[color:var(--ivory)] px-4">
              ...
              <input name="q" defaultValue={q ?? ""} placeholder="姓名 / 邮箱 / 电话 / 内容 / 备注" className="min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-[color:var(--muted)]/70" />
            </div>
          </label>
      ...
      <section className="mt-6 overflow-hidden border border-[color:var(--border)] bg-[color:var(--paper)]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[color:var(--border)] p-5">
          <h2 className="font-serif text-2xl font-light">留言记录</h2>
          ...
          <article key={row.id.toString()} className="grid gap-5 border-b border-[color:var(--border)] p-5 last:border-b-0 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="border border-[color:var(--border)] px-2 py-1 text-xs text-[color:var(--muted)]">{row.category ?? "未分类"}</span>
              ...
              <div className="mt-3 grid gap-2 text-sm leading-6 text-[color:var(--muted)] md:grid-cols-2">
                ...
                <p>提交时间：{formatDate(row.createdAt)}</p>
            ...
            <form action={updateContactAction} className="grid content-start gap-3 border border-[color:var(--border)] bg-white p-4">
              ...
              <label className="grid gap-2 text-sm">
                <span className="text-[color:var(--muted)]">回复备注</span>
              ...
              <label className="grid gap-2 text-sm">
                ...
                <textarea name="adminNote" defaultValue={row.adminNote ?? ""} disabled={readOnly} rows={4} className="w-full resize-y border border-[color:var(--border)] bg-[color:var(--ivory)] px-3 py-2 text-base leading-7 outline-none disabled:opacity-70" placeholder="记录电话联系、报价方向、转交人员等内部信息。" />
              </label>
              {!readOnly ? <button className="min-h-11 border border-[color:var(--gold)] bg-[color:var(--gold)] px-4 text-sm text-white">保存状态与备注</button> : null}
            </form>
          </article>
        )) : (
          <p className="p-8 text-sm text-[color:var(--muted)]">暂无符合筛选条件的联系留言。</p>
        )}
      </section>

      <nav className="mt-5 flex flex-wrap items-center justify-between gap-3">
        <Link href={buildHref("/admin/contacts", normalizedParams, Math.max(1, page - 1))} className={`min-h-11 border border-[color:var(--border)] px-4 py-3 text-sm ${page <= 1 ? "pointer-events-none opacity-50" : "text-[color:var(--gold-dark)]"}`}>上一页</Link>
        <Link href={buildHref("/admin/contacts", normalizedParams, Math.min(pageCount, page + 1))} className={`min-h-11 border border-[color:var(--border)] px-4 py-3 text-sm ${page >= pageCount ? "pointer-events-none opacity-50" : "text-[color:var(--gold-dark)]"}`}>下一页</Link>
      </nav>
    </AdminShell>
  );
}
