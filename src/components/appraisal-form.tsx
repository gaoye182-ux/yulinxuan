...
import { submitAppraisalAction } from "@/lib/appraisal-actions";
import type { Language } from "@/lib/i18n";
import { localizedPath } from "@/lib/navigation";

export type AppraisalFormCopy = {
  form: {
    title: string;
    lead: string;
    required: string;
...
type ErrorMap = Partial<Record<keyof FormState | "files", string>>;
...
export function AppraisalForm({ copy, lang }: AppraisalFormProps) {
  const [form, setForm] = useState<FormState>({
    ...initialForm,
    type: copy.options.types[0],
    category: copy.options.categories[0]
  });
  const [files, setFiles] = useState<File[]>([]);
  const [errors, setErrors] = useState<ErrorMap>({});
  const [state, formAction, pending] = useActionState(submitAppraisalAction, { ok: false });
  ...
  function validate(nextFiles = files) {
    ...
    if (nextFiles.length > 5) {
      nextErrors.files = copy.form.errors.fileCount;
    } else {
      const invalidSize = nextFiles.some((file) => file.size > 5 * 1024 * 1024);
      const invalidType = nextFiles.some((file) => {
        const extensionOk = /\.(jpe?g|png|heic|heif)$/i.test(file.name);
        return !acceptedTypes.includes(file.type) && !extensionOk;
      });

      if (invalidSize) {
        nextErrors.files = copy.form.errors.fileSize;
      } else if (invalidType) {
        nextErrors.files = copy.form.errors.fileType;
      }
    }
  ...
  return (
    <form action={formAction} onSubmit={handleSubmit} noValidate className="border border-[color:var(--border)] bg-[color:var(--paper)] p-5 md:p-8">
      ...
      <div className="border-b border-[color:var(--border)] pb-6">
        <p className="text-xs tracking-[0.3em] text-[color:var(--gold)]">REQUEST FORM</p>
        <h2 className="mt-3 font-serif text-3xl font-light text-[color:var(--ink)]">{copy.form.title}</h2>
      ...
      {state.ok ? (
        <div className="mt-6 border border-[color:var(--gold)] bg-[rgba(176,141,87,0.08)] p-5 text-[color:var(--ink)]">
          ...
          <p className="mt-3 text-sm leading-7 text-[color:var(--muted)]">
            {copy.form.successText}{state.requestNo ? ` (${state.requestNo})` : ""}
          </p>
        </div>
      ) : null}
      ...
      <div className="mt-7 grid gap-5 md:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm text-[color:var(--ink)]">
          ...
          <div className="relative">
            <MapPin aria-hidden size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-[color:var(--gold-dark)]" />
            <input
              name="region"
              value={form.region}
              onChange={(event) => updateField("region", event.target.value)}
              placeholder={copy.form.regionPlaceholder}
              className={`${inputClass()} pl-11`}
        ...
        <label className="grid gap-2 text-sm text-[color:var(--ink)]">
          <span>{copy.form.preferredDate}</span>
          <div className="relative">
            <CalendarDays aria-hidden size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-[color:var(--gold-dark)]" />
            <input
              name="preferredDate"
        ...
        <div className="grid gap-2 text-sm text-[color:var(--ink)] md:col-span-2">
          <span>{copy.form.upload}</span>
          <label
            className={`flex min-h-36 cursor-pointer flex-col items-center justify-center gap-3 border border-dashed px-4 py-6 text-center transition ${
              errors.files
                ? "border-[color:var(--red-seal)] bg-[rgba(143,46,36,0.04)]"
                : "border-[color:var(--border)] bg-[color:var(--ivory)] hover:border-[color:var(--gold)]"
            }`}
          >
            <Upload aria-hidden size={28} className="text-[color:var(--gold-dark)]" />
            <span className="text-base text-[color:var(--ink)]">{copy.form.uploadHelp}</span>
            <span className="text-xs leading-5 text-[color:var(--muted)]">JPG / PNG / HEIC, 5MB x 5</span>
            <input
              type="file"
              name="images"
          ...
          {selectedFileNames.length ? (
            <ul className="grid gap-2">
              {selectedFileNames.map((name) => (
                <li key={name} className="flex min-h-11 items-center gap-2 border border-[color:var(--border)] bg-[color:var(--ivory)] px-3 text-sm text-[color:var(--muted)]">
                  <FileImage aria-hidden size={16} className="shrink-0 text-[color:var(--gold-dark)]" />
